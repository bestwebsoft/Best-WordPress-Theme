		...
		while ( have_posts() ) {
			...
			<section id="best-post-<?php the_ID(); ?>"  <?php post_class( 'best-post' ); ?>>
					...
					<h2 class="best-title"><a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php _e( 'Permalink to', 'best' ); the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
				 	<p class="best-p-post-data">
				 		...
				 		<a href="<?php echo esc_url( wp_get_attachment_url() ); ?>" target="_blank">
				 			<?php $metadata = wp_get_attachment_metadata(); echo $metadata['width']; ?> &times;<?php echo $metadata['height']; ?>
				 		</a>
				 		<span class="post-data"> <?php _e( 'in', 'best' ); ?> </span>
				 		<a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php _e( 'Return to', 'best' ); echo ' ' . esc_attr( strip_tags( get_the_title( $post->post_parent ) ) ); ?>">
				...
				<div class="entry">
					<article class="entry-attachment">
						<div class="attachment"> <?php 
							...
							<a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"> <?php 
								$attachment_size = apply_filters( 'best_attachment_size', array( 654, 99999 ) );
					...
					<article class="entry-description"> <?php 
						the_content();
						wp_link_pages( 
							array( 
